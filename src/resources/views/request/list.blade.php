...
@section('content')
    <article class="attendance-list-container">
        ...
        <header class="attendance-list-header">
            <a href="{{ route('stamp_request.list', ['status' => 'pending']) }}"
                class="tab {{ $status === 'pending' ? 'active' : '' }}">承認待ち</a>

            <a href="{{ route('stamp_request.list', ['status' => 'approved']) }}"
                ...
                <tbody>
                    @foreach ($requests as $req)
                        <tr>
                            ...
                            <td class="attendance-table__data">
                                <a href="{{ route('attendance.show', $req->attendance_id) }}" class="detail-link">
                                    詳細
                                </a>
                            </td>
                        </tr>
